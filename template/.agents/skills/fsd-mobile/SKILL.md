...
name: fsd-mobile
description: >
  Unofficial Feature-Sliced Design (FSD) v1 skill for applying the methodology
  to react-native projects. Use when the task involves organizing project structure
...
FSD v1 core principle: **"Start simple, extract when needed."**

Place code in `screens/` first. Duplication across screens is acceptable and does
...
`screens/`, and `app/`. Add `widgets/`, `features/`, `entities/` only when they
...
widgets/   → Large composite UI blocks reused across multiple screens
...
import { useUser } from "@/entities/user"; // screens → entities
...
- Used in only one screen → keep it in that `screens/` slice.
- Used in 2+ screens but duplication is manageable → keeping separate copies
  in each screen is also valid.
- An entity or feature used in only one screen → keep it in that screen
...
- Yes → `features/`
- Uncertain, single use, or speculative reuse → keep in the screen.
...
**Golden Rule: When in doubt, keep it in `screens/`. Extract only when the
...
| --------------------- | ------------------------------------------- | ------------------------------------- |
| User profile form     | `screens/profile/ui/ProfileForm.tsx`          | `features/profile-form/`              |
| Product card          | `screens/products/ui/ProductCard.tsx`         | `entities/product/ui/ProductCard.tsx` |
| Product data fetching | `screens/product-detail/api/fetch-product.ts` | `entities/product/api/`               |
| Auth token/session    | `shared/auth/` (always)                     | `shared/auth/` (always)               |
| Auth login form       | `screens/login/ui/LoginForm.tsx`              | `features/auth/`                      |
...
| Modal content         | `screens/[screen]/ui/SomeModal.tsx`             |                                       |
...
Place code in `screens/` first. Extract to lower layers only when truly needed.
Extraction is a design decision that affects the whole project, so the
threshold should be high.

**What stays in screens:**

- Large UI blocks used only in one screen
- Screen-specific forms, validation, data fetching, state management
- Screen-specific business logic and API integrations
- Code that looks reusable but is simpler to keep local

**Evolution pattern:** Start with everything in `screens/profile/`. When the
same user data is being consumed by another screen (not hypothetically),
extract the shared model to `entities/user/`. Keep screen-specific API calls
and UI in the screen.
...
1. **Start without entities.** `shared/` + `screens/` + `app/` is valid FSD.
   Thin-client apps rarely need entities.
2. **Do not split slices prematurely.** Keep code in screens. Extract to
...
// + features/  ← User interactions currently reused across multiple screens
// + entities/  ← Domain models currently reused across screens or features
```
...
- **`insignificant-slice`**: Suggests merging an entity/feature into its screen
  if only one screen uses it.
...
- **Do not extract single-use code.** A feature or entity used by only one
  screen should stay in that screen.
...
- **Be cautious adding UI to entities.** Entity UI tempts cross-imports from
  ...
  higher layers (features, widgets, screens), never from other entities.
...
- **Strategy C: Compose from upper layer (IoC).** The parent (screens or app)
...
- **Screens, Widgets, Features, Entities**: Slices first, then segments inside
  each slice.
...
- **Create entities when**: the same business domain model is currently
  used across multiple screens, features, or widgets, with stable
  boundaries.
- **Create features when**: the same user interaction is currently used
  across multiple screens or widgets, with stable boundaries.
...
- **When integrating FSD with a specific framework** (Next.js with App Router
  or Screens Router, Nuxt, Vite, CRA, Astro) for wiring routes to FSD screens,
